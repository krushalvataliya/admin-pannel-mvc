...
class Controller_Salesman_price extends Controller_Core_Action
{
	public function gridAction()
	{
		...
		$modelSalesmanPrice =Ccc::getModel('SalesmanPrice_row');
		...
		$modelSalesmanPrice =Ccc::getModel('product_row');
		$products = $modelSalesmanPrice->fetchAll($sql);

		$this->getView()->setTemplete('Salesman/price/grid.phtml')->setData(['salesmen' => $salesmen,'products' => $products]);
		$this->render();
	}
	...
	public function updateAction()
	{
		try
		{
			...
			{
			$modelSalesmanPrice1 =Ccc::getModel('SalesmanPrice_row');
